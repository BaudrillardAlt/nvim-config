...
local function yank_all_diagnostics()
  local diags = vim.diagnostic.get() -- all buffers
  local lines = {}

  for _, d in ipairs(diags) do
    local fname = vim.api.nvim_buf_get_name(d.bufnr)
    local rel = vim.fn.fnamemodify(fname, ":.") -- relative to cwd
...
nmap("<leader><leader>x", yank_line_diagnostics, { desc = "Yank line diagnostics to clipboard" })
nmap("<leader><leader>d", yank_all_diagnostics, { desc = "Yank all diagnostics to clipboard" })
